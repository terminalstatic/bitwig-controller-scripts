...
host.defineController("Keith McMillen", "QuNexusMulti", "0.1", "a535d5ee-82ac-429a-9517-67f82048e02d", "terminal_static");
...
function init() {
    ...
    let noteIn = host.getMidiInPort(0).createNoteInput("All Channels", "??????");
    noteIn.setShouldConsumeEvents(false);

    for (let i = 0; i < 16; i++) {
        let hex = i.toString(16).toUpperCase();
        let noteIn = host.getMidiInPort(0).createNoteInput("Channel " + (i + 1), "?" + i.toString(16) + "????");
        noteIn.setShouldConsumeEvents(false);
    }
